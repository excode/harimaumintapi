    ...
    var sql = require('yesql').pg
      ...
      exports.findById = (id,extraFields) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from withdraw where id=:id '+searchConditions+' LIMIT 1'
      ...
      exports.findOne = (querys) => {
        ...
        ;(async () => {
            ...
            const client = await pool.connect()
            try {
                const queryText = 'SELECT * from withdraw '+condition+' LIMIT 1'
                const list = await client.query(sql(queryText)(vals))
                resolve(list.rows[0]);
            } catch (e) {
    ...
    vals['comment'] = body.comment ; 
          ...
          ;(async () => {
            ...
            try {
                ...
                const queryText = 'INSERT INTO withdraw('+column+') VALUES('+params+') RETURNING id ';
        ...
        if(Array.isArray(query.amount_array) && query.amount_array.length==2){
            ...
            ;(async () => {
                ...
                try {
                    ...
                    const queryText = 'SELECT * from withdraw' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage+' OFFSET '+offset;
                    const list = await client.query(sql(queryText)(vals));
                    const queryTextCount = 'SELECT COUNT(*) as total from withdraw' +searchConditions
      ...
      exports.listAll = (querys={} ) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from withdraw' +searchConditions+'  LIMIT '+perPage;
                const list = await client.query(sql(queryText)(vals));
  ...
  exports.listSuggestions = (query) => {
    ...
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from withdraw' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage;
            const list = await client.query(sql(queryText)(vals));
           resolve( list.rows);
      ...
      exports.patchWithdraw = (id, reqData,extraFields={}) => {
        var body = reqData;
    ...
    vals['comment'] = body.comment ;  
        ...
        ;(async () => {
          ...
          try {
              ...
              await client.query('BEGIN');
              const queryText = 'UPDATE withdraw SET '+column+'  WHERE id =:id'+searchConditions
              const updated = await client.query(sql(queryText)(vals));
              //console.log(sql(queryText)(vals))
              await client.query('COMMIT');
              //VCache.resetCache("withdraw_list");
              resolve(updated);
          } catch (e) {
              await client.query('ROLLBACK')
              console.log(e);
      ...
      exports.removeById = (withdrawId,extraFields={}) => {
          return new Promise((resolve, reject) => {
            ;(async () => {
                const extra = queryFormatter(extraFields)
                let vals= {id:withdrawId};
                let searchConditions=""
                if(extra.cols.length>0){
                    vals={...vals,...extra.vals};
                    searchConditions=" AND " + extra.cols.join(" AND ")
                ...
                try {
                    await client.query('BEGIN')
                    const queryText = 'DELETE FROM withdraw  WHERE  id=:id '+searchConditions
