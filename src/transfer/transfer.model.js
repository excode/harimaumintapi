    ...
    var sql = require('yesql').pg
      ...
      exports.findById = (id,extraFields) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from transfer where id=:id '+searchConditions+' LIMIT 1'
      ...
      exports.findOne = (querys) => {
        ...
        ;(async () => {
            ...
            const client = await pool.connect()
            try {
                const queryText = 'SELECT * from transfer '+condition+' LIMIT 1'
                const list = await client.query(sql(queryText)(vals))
                resolve(list.rows[0]);
            } catch (e) {
...
if(body.comments!=undefined){
    ...
    vals['comments'] = body.comments ; 
          ...
          ;(async () => {
            ...
            try {
                ...
                const queryText = 'INSERT INTO transfer('+column+') VALUES('+params+') RETURNING id ';
    ...
    if(query.sourcewallet){
        let sourcewallet_ = queryBuilder_string(query,"sourcewallet","sourcewallet")
        cols.push(sourcewallet_);
        vals['sourcewallet'] = query.sourcewallet;
       
    }
    if(query.sourcewallet_array){
        if(Array.isArray(query.sourcewallet_array) && query.sourcewallet_array.length>1){
            cols.push(" sourcewallet = ANY(:sourcewallet) ");
            const sourcewalletString = query.sourcewallet_array.map((data) => data);
            vals['sourcewallet'] = sourcewalletString;
        }
    }
        ...
        if(Array.isArray(query.status_array) && query.status_array.length>1){
            ...
            ;(async () => {
                ...
                try {
                    ...
                    const queryText = 'SELECT * from transfer' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage+' OFFSET '+offset;
                    const list = await client.query(sql(queryText)(vals));
                    const queryTextCount = 'SELECT COUNT(*) as total from transfer' +searchConditions
      ...
      exports.listAll = (querys={} ) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from transfer' +searchConditions+'  LIMIT '+perPage;
                const list = await client.query(sql(queryText)(vals));
  ...
  exports.listSuggestions = (query) => {
    ...
    let sortBy='id';
    let sortDirection='ASC';
    let keyConditions =` WHERE  (sourcewallet LIKe '%' || :search || '%' 
 OR status LIKe '%' || :search || '%' )`;
    ...
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from transfer' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage;
            const list = await client.query(sql(queryText)(vals));
           resolve( list.rows);
      ...
      exports.patchTransfer = (id, reqData,extraFields={}) => {
        var body = reqData;
    ...
    vals['comments'] = body.comments ;  
        ...
        ;(async () => {
          ...
          try {
              ...
              await client.query('BEGIN');
              const queryText = 'UPDATE transfer SET '+column+'  WHERE id =:id'+searchConditions
              const updated = await client.query(sql(queryText)(vals));
              //console.log(sql(queryText)(vals))
              await client.query('COMMIT');
              //VCache.resetCache("transfer_list");
              resolve(updated);
          } catch (e) {
              await client.query('ROLLBACK')
              console.log(e);
      ...
      exports.removeById = (transferId,extraFields={}) => {
          return new Promise((resolve, reject) => {
            ;(async () => {
                const extra = queryFormatter(extraFields)
                let vals= {id:transferId};
                let searchConditions=""
                if(extra.cols.length>0){
                    vals={...vals,...extra.vals};
                    searchConditions=" AND " + extra.cols.join(" AND ")
                ...
                try {
                    await client.query('BEGIN')
                    const queryText = 'DELETE FROM transfer  WHERE  id=:id '+searchConditions
