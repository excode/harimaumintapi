    ...
    var sql = require('yesql').pg
      ...
      exports.findById = (id,extraFields) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from exchange where id=:id '+searchConditions+' LIMIT 1'
      ...
      exports.findOne = (querys) => {
        ...
        ;(async () => {
            ...
            const client = await pool.connect()
            try {
                const queryText = 'SELECT * from exchange '+condition+' LIMIT 1'
                const list = await client.query(sql(queryText)(vals))
                resolve(list.rows[0]);
            } catch (e) {
...
if(body.sourcewallet!=undefined){
    cols.push("sourcewallet");
    param.push(":sourcewallet");
    vals['sourcewallet'] = body.sourcewallet ; 
}
...
if(body.sourcewallettype!=undefined){
    cols.push("sourcewallettype");
    param.push(":sourcewallettype");
    vals['sourcewallettype'] = body.sourcewallettype ; 
}
    ...
    vals['status'] = body.status ; 
          ...
          ;(async () => {
            ...
            try {
                ...
                const queryText = 'INSERT INTO exchange('+column+') VALUES('+params+') RETURNING id ';
        ...
        if(Array.isArray(query.amount_array) && query.amount_array.length==2){
            ...
            ;(async () => {
                ...
                try {
                    ...
                    const queryText = 'SELECT * from exchange' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage+' OFFSET '+offset;
                    const list = await client.query(sql(queryText)(vals));
                    const queryTextCount = 'SELECT COUNT(*) as total from exchange' +searchConditions
      ...
      exports.listAll = (querys={} ) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from exchange' +searchConditions+'  LIMIT '+perPage;
                const list = await client.query(sql(queryText)(vals));
  ...
  exports.listSuggestions = (query) => {
    ...
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from exchange' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage;
            const list = await client.query(sql(queryText)(vals));
           resolve( list.rows);
      ...
      exports.patchExchange = (id, reqData,extraFields={}) => {
        var body = reqData;
...
if(body.sourcewallet!=undefined){
    cols.push("sourcewallet = :sourcewallet");
    vals['sourcewallet'] = body.sourcewallet ;  
}
...
if(body.sourcewallettype!=undefined){
    cols.push("sourcewallettype = :sourcewallettype");
    vals['sourcewallettype'] = body.sourcewallettype ;  
}
    ...
    vals['status'] = body.status ;  
        ...
        ;(async () => {
          ...
          try {
              ...
              await client.query('BEGIN');
              const queryText = 'UPDATE exchange SET '+column+'  WHERE id =:id'+searchConditions
              const updated = await client.query(sql(queryText)(vals));
              //console.log(sql(queryText)(vals))
              await client.query('COMMIT');
              //VCache.resetCache("exchange_list");
              resolve(updated);
          } catch (e) {
              await client.query('ROLLBACK')
              console.log(e);
      ...
      exports.removeById = (exchangeId,extraFields={}) => {
          return new Promise((resolve, reject) => {
            ;(async () => {
                const extra = queryFormatter(extraFields)
                let vals= {id:exchangeId};
                let searchConditions=""
                if(extra.cols.length>0){
                    vals={...vals,...extra.vals};
                    searchConditions=" AND " + extra.cols.join(" AND ")
                ...
                try {
                    await client.query('BEGIN')
                    const queryText = 'DELETE FROM exchange  WHERE  id=:id '+searchConditions
