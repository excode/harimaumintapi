    ...
    var sql = require('yesql').pg
      ...
      exports.findById = (id,extraFields) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from history where id=:id '+searchConditions+' LIMIT 1'
      ...
      exports.findOne = (querys) => {
        ...
        ;(async () => {
            ...
            const client = await pool.connect()
            try {
                const queryText = 'SELECT * from history '+condition+' LIMIT 1'
                const list = await client.query(sql(queryText)(vals))
                resolve(list.rows[0]);
            } catch (e) {
    ...
    vals['status'] = body.status ; 
          ...
          ;(async () => {
            ...
            try {
                ...
                const queryText = 'INSERT INTO history('+column+') VALUES('+params+') RETURNING id ';
        ...
        if(Array.isArray(query.subject_array) && query.subject_array.length>1){
            ...
            ;(async () => {
                ...
                try {
                    ...
                    const queryText = 'SELECT * from history' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage+' OFFSET '+offset;
                    const list = await client.query(sql(queryText)(vals));
                    const queryTextCount = 'SELECT COUNT(*) as total from history' +searchConditions
      ...
      exports.listAll = (querys={} ) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from history' +searchConditions+'  LIMIT '+perPage;
                const list = await client.query(sql(queryText)(vals));
  ...
  exports.listSuggestions = (query) => {
    ...
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from history' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage;
            const list = await client.query(sql(queryText)(vals));
           resolve( list.rows);
      ...
      exports.patchHistory = (id, reqData,extraFields={}) => {
        var body = reqData;
    ...
    vals['status'] = body.status ;  
        ...
        ;(async () => {
          ...
          try {
              ...
              await client.query('BEGIN');
              const queryText = 'UPDATE history SET '+column+'  WHERE id =:id'+searchConditions
              const updated = await client.query(sql(queryText)(vals));
              //console.log(sql(queryText)(vals))
              await client.query('COMMIT');
              //VCache.resetCache("history_list");
              resolve(updated);
          } catch (e) {
              await client.query('ROLLBACK')
              console.log(e);
      ...
      exports.removeById = (historyId,extraFields={}) => {
          return new Promise((resolve, reject) => {
            ;(async () => {
                const extra = queryFormatter(extraFields)
                let vals= {id:historyId};
                let searchConditions=""
                if(extra.cols.length>0){
                    vals={...vals,...extra.vals};
                    searchConditions=" AND " + extra.cols.join(" AND ")
                ...
                try {
                    await client.query('BEGIN')
                    const queryText = 'DELETE FROM history  WHERE  id=:id '+searchConditions
