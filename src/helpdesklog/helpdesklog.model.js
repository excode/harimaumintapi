    ...
    var sql = require('yesql').pg
      ...
      exports.findById = (id,extraFields) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from helpdesklog where id=:id '+searchConditions+' LIMIT 1'
      ...
      exports.findOne = (querys) => {
        ...
        ;(async () => {
            ...
            const client = await pool.connect()
            try {
                const queryText = 'SELECT * from helpdesklog '+condition+' LIMIT 1'
                const list = await client.query(sql(queryText)(vals))
                resolve(list.rows[0]);
            } catch (e) {
      ...
      exports.createHelpdesklog = (body) => {
...
if(body.comments!=undefined){
    cols.push("comments");
    param.push(":comments");
    vals['comments'] = body.comments ; 
}
                         
if(body.documents!=undefined){
    cols.push("documents");
    param.push(":documents");
    vals['documents'] = body.documents ; 
}
          ...
          ;(async () => {
            ...
            try {
                ...
                const queryText = 'INSERT INTO helpdesklog('+column+') VALUES('+params+') RETURNING id ';
    ...
    if(query.subject){
        let subject_ = queryBuilder_string(query,"subject","subject")
        cols.push(subject_);
        vals['subject'] = query.subject;
       
    }
    if(query.subject_array){
        if(Array.isArray(query.subject_array) && query.subject_array.length>1){
            cols.push(" subject = ANY(:subject) ");
            const subjectString = query.subject_array.map((data) => data);
            vals['subject'] = subjectString;
        }
    }
            ...
            ;(async () => {
                ...
                try {
                    ...
                    const queryText = 'SELECT * from helpdesklog' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage+' OFFSET '+offset;
                    const list = await client.query(sql(queryText)(vals));
                    const queryTextCount = 'SELECT COUNT(*) as total from helpdesklog' +searchConditions
      ...
      exports.listAll = (querys={} ) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from helpdesklog' +searchConditions+'  LIMIT '+perPage;
                const list = await client.query(sql(queryText)(vals));
  ...
  exports.listSuggestions = (query) => {
    ...
    let sortBy='id';
    let sortDirection='ASC';
    let keyConditions =` WHERE  (subject LIKe '%' || :search || '%' )`;
    ...
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from helpdesklog' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage;
            const list = await client.query(sql(queryText)(vals));
           resolve( list.rows);
      ...
      exports.patchHelpdesklog = (id, reqData,extraFields={}) => {
        var body = reqData;
...
if(body.subject!=undefined){
    cols.push("subject = :subject");
    vals['subject'] = body.subject ;  
}
                             
if(body.comments!=undefined){
    cols.push("comments = :comments");
    vals['comments'] = body.comments ;  
}
                             
if(body.documents!=undefined){
    cols.push("documents = :documents");
    vals['documents'] = body.documents ;  
}
        vals= {...vals,id:id}; 
        var column = cols.join(',');
        ...
        ;(async () => {
          ...
          try {
              ...
              await client.query('BEGIN');
              const queryText = 'UPDATE helpdesklog SET '+column+'  WHERE id =:id'+searchConditions
              const updated = await client.query(sql(queryText)(vals));
              //console.log(sql(queryText)(vals))
              await client.query('COMMIT');
              //VCache.resetCache("helpdesklog_list");
              resolve(updated);
          } catch (e) {
              await client.query('ROLLBACK')
              console.log(e);
      ...
      exports.removeById = (helpdesklogId,extraFields={}) => {
          return new Promise((resolve, reject) => {
            ;(async () => {
                const extra = queryFormatter(extraFields)
                let vals= {id:helpdesklogId};
                let searchConditions=""
                if(extra.cols.length>0){
                    vals={...vals,...extra.vals};
                    searchConditions=" AND " + extra.cols.join(" AND ")
                ...
                try {
                    await client.query('BEGIN')
                    const queryText = 'DELETE FROM helpdesklog  WHERE  id=:id '+searchConditions
    ...
    exports.uploadFile = (req) => {
        return new Promise(async(resolve, reject) => {
            ...
            ;(async () => {
                ...
                try {
              await client.query('BEGIN');
              const queryText = 'UPDATE helpdesklog SET '+colName+'=:0  where id =:1';
