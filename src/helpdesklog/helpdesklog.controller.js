const HelpdesklogModel = require('./helpdesklog.model');
  ...
  exports.insert = (req, res) => {
        ...
        req.body.id=req.jwt.id
            ...
            HelpdesklogModel.createHelpdesklog(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      HelpdesklogModel.list(limit, page,req.query)
          .then((result) => {
  ...
  exports.listAll = (req, res ) => {
     ...
     */
    HelpdesklogModel.listAll(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    HelpdesklogModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    HelpdesklogModel.findById(req.params.helpdesklogId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      HelpdesklogModel.patchHelpdesklog(req.params.helpdesklogId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    HelpdesklogModel.removeById(req.params.helpdesklogId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
...
exports.uploadfile=(req,res)=>{
    HelpdesklogModel.uploadFile(req)
        .then((result) => {
