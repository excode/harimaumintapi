    ...
    var sql = require('yesql').pg
      ...
      exports.findById = (id,extraFields) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from deposit where id=:id '+searchConditions+' LIMIT 1'
      ...
      exports.findOne = (querys) => {
        ...
        ;(async () => {
            ...
            const client = await pool.connect()
            try {
                const queryText = 'SELECT * from deposit '+condition+' LIMIT 1'
                const list = await client.query(sql(queryText)(vals))
                resolve(list.rows[0]);
            } catch (e) {
...
if(body.status!=undefined){
    ...
    vals['status'] = body.status ; 
          ...
          ;(async () => {
            ...
            try {
                ...
                const queryText = 'INSERT INTO deposit('+column+') VALUES('+params+') RETURNING id ';
    ...
    if(query.method){
        let method_ = queryBuilder_string(query,"method","method")
        cols.push(method_);
        vals['method'] = query.method;
       
    }
    if(query.method_array){
        if(Array.isArray(query.method_array) && query.method_array.length>1){
            cols.push(" method = ANY(:method) ");
            const methodString = query.method_array.map((data) => data);
            vals['method'] = methodString;
        }
    }
            ...
            ;(async () => {
                ...
                try {
                    ...
                    const queryText = 'SELECT * from deposit' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage+' OFFSET '+offset;
                    const list = await client.query(sql(queryText)(vals));
                    const queryTextCount = 'SELECT COUNT(*) as total from deposit' +searchConditions
      ...
      exports.listAll = (querys={} ) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from deposit' +searchConditions+'  LIMIT '+perPage;
                const list = await client.query(sql(queryText)(vals));
  ...
  exports.listSuggestions = (query) => {
    ...
    let sortBy='id';
    let sortDirection='ASC';
    let keyConditions =` WHERE  (method LIKe '%' || :search || '%' )`;
    ...
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from deposit' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage;
            const list = await client.query(sql(queryText)(vals));
           resolve( list.rows);
      ...
      exports.patchDeposit = (id, reqData,extraFields={}) => {
        var body = reqData;
    ...
    vals['status'] = body.status ;  
        ...
        ;(async () => {
          ...
          try {
              ...
              await client.query('BEGIN');
              const queryText = 'UPDATE deposit SET '+column+'  WHERE id =:id'+searchConditions
              const updated = await client.query(sql(queryText)(vals));
              //console.log(sql(queryText)(vals))
              await client.query('COMMIT');
              //VCache.resetCache("deposit_list");
              resolve(updated);
          } catch (e) {
              await client.query('ROLLBACK')
              console.log(e);
      ...
      exports.removeById = (depositId,extraFields={}) => {
          return new Promise((resolve, reject) => {
            ;(async () => {
                const extra = queryFormatter(extraFields)
                let vals= {id:depositId};
                let searchConditions=""
                if(extra.cols.length>0){
                    vals={...vals,...extra.vals};
                    searchConditions=" AND " + extra.cols.join(" AND ")
                ...
                try {
                    await client.query('BEGIN')
                    const queryText = 'DELETE FROM deposit  WHERE  id=:id '+searchConditions
    ...
    exports.uploadFile = (req) => {
        return new Promise(async(resolve, reject) => {
            ...
            ;(async () => {
                ...
                try {
              await client.query('BEGIN');
              const queryText = 'UPDATE deposit SET '+colName+'=:0  where id =:1';
