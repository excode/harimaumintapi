const ProfitModel = require('./profit.model');
  ...
  exports.insert = (req, res) => {
        ...
        req.body.id=req.jwt.id
            ...
            ProfitModel.createProfit(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      ProfitModel.list(limit, page,req.query)
          .then((result) => {
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    ProfitModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    ProfitModel.findById(req.params.profitId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      ProfitModel.patchProfit(req.params.profitId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    ProfitModel.removeById(req.params.profitId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
