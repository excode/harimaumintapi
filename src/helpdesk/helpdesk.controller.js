const HelpdeskModel = require('./helpdesk.model');
  ...
  exports.insert = (req, res) => {
        ...
        req.body.id=req.jwt.id
            ...
            HelpdeskModel.createHelpdesk(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      HelpdeskModel.list(limit, page,req.query)
          .then((result) => {
  ...
  exports.listAll = (req, res ) => {
     ...
     */
    HelpdeskModel.listAll(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    HelpdeskModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    HelpdeskModel.findById(req.params.helpdeskId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      HelpdeskModel.patchHelpdesk(req.params.helpdeskId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    HelpdeskModel.removeById(req.params.helpdeskId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
