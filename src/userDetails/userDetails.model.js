    ...
    var sql = require('yesql').pg
      ...
      exports.findById = (id,extraFields) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from userdetails where id=:id '+searchConditions+' LIMIT 1'
      ...
      exports.findOne = (querys) => {
        ...
        ;(async () => {
            ...
            const client = await pool.connect()
            try {
                const queryText = 'SELECT * from userdetails '+condition+' LIMIT 1'
                const list = await client.query(sql(queryText)(vals))
                resolve(list.rows[0]);
            } catch (e) {
...
exports.findByIcpassport = (icpassport) => {
    return new Promise((resolve, reject) => {
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from userdetails where icpassport=:icpassport LIMIT 1'
            const list = await client.query(sql(queryText)({icpassport:icpassport}))
            resolve(list.rows[0]);
      ...
      exports.createUserDetails = async(body) => {
...
if(body.icpassport!=undefined){
    cols.push("icpassport");
    param.push(":icpassport");
    vals['icpassport'] = body.icpassport ; 
}
                         
if(body.photo!=undefined){
    cols.push("photo");
    param.push(":photo");
    vals['photo'] = body.photo ; 
}
                         
if(body.icdocument!=undefined){
    cols.push("icdocument");
    param.push(":icdocument");
    vals['icdocument'] = body.icdocument ; 
}
        ...
        let   icpassportCHeck =await this.findOne({"icpassport":body.icpassport})
          if(icpassportCHeck ) {
            reject("icpassport exists");
            return;
          }
        
                await client.query('BEGIN');
                const queryText = 'INSERT INTO userdetails('+column+') VALUES('+params+') RETURNING id ';
    ...
    if(query.icpassport){
        let icpassport_ = queryBuilder_string(query,"icpassport","icpassport")
        cols.push(icpassport_);
        vals['icpassport'] = query.icpassport;
       
    }
    if(query.icpassport_array){
        if(Array.isArray(query.icpassport_array) && query.icpassport_array.length>1){
            cols.push(" icpassport = ANY(:icpassport) ");
            const icpassportString = query.icpassport_array.map((data) => data);
            vals['icpassport'] = icpassportString;
        }
    }
            ...
            ;(async () => {
                ...
                try {
                    ...
                    const queryText = 'SELECT * from userdetails' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage+' OFFSET '+offset;
                    const list = await client.query(sql(queryText)(vals));
                    const queryTextCount = 'SELECT COUNT(*) as total from userdetails' +searchConditions
      ...
      exports.listAll = (querys={} ) => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const queryText = 'SELECT * from userdetails' +searchConditions+'  LIMIT '+perPage;
                const list = await client.query(sql(queryText)(vals));
  ...
  exports.listSuggestions = (query) => {
    ...
    let sortBy='id';
    let sortDirection='ASC';
    let keyConditions =` WHERE  (address LIKe '%' || :search || '%' 
 OR icpassport LIKe '%' || :search || '%' )`;
    ...
    ;(async () => {
        ...
        try {
            ...
            const queryText = 'SELECT * from userdetails' +searchConditions+' order by '+sortBy+' '+sortDirection+' LIMIT '+perPage;
            const list = await client.query(sql(queryText)(vals));
           resolve( list.rows);
      ...
      exports.patchUserDetails = async(id, reqData,extraFields={}) => {
        var body = reqData;
...
if(body.icpassport!=undefined){
    cols.push("icpassport = :icpassport");
    vals['icpassport'] = body.icpassport ;  
}
                             
if(body.photo!=undefined){
    cols.push("photo = :photo");
    vals['photo'] = body.photo ;  
}
                             
if(body.icdocument!=undefined){
    cols.push("icdocument = :icdocument");
    vals['icdocument'] = body.icdocument ;  
}
        vals= {...vals,id:id}; 
        var column = cols.join(',');
        ...
        if(body.icpassport!=undefined){
        let   icpassportCHeck =await this.findOne({id:{ne:id},"icpassport":body.icpassport})
          if(icpassportCHeck ) {
            reject("icpassport exists");
            return;
          }
              ...
              await client.query('BEGIN');
              const queryText = 'UPDATE userdetails SET '+column+'  WHERE id =:id'+searchConditions
              const updated = await client.query(sql(queryText)(vals));
              //console.log(sql(queryText)(vals))
              await client.query('COMMIT');
              //VCache.resetCache("userdetails_list");
              resolve(updated);
          } catch (e) {
              await client.query('ROLLBACK')
              console.log(e);
      ...
      exports.removeById = (userdetailsId,extraFields={}) => {
          return new Promise((resolve, reject) => {
            ;(async () => {
                const extra = queryFormatter(extraFields)
                let vals= {id:userdetailsId};
                let searchConditions=""
                if(extra.cols.length>0){
                    vals={...vals,...extra.vals};
                    searchConditions=" AND " + extra.cols.join(" AND ")
                ...
                try {
                    await client.query('BEGIN')
                    const queryText = 'DELETE FROM userdetails  WHERE  id=:id '+searchConditions
    ...
    exports.uploadFile = (req) => {
        return new Promise(async(resolve, reject) => {
            ...
            ;(async () => {
                ...
                try {
              await client.query('BEGIN');
              const queryText = 'UPDATE userdetails SET '+colName+'=:0  where id =:1';
