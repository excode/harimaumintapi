const UserDetailsModel = require('./userDetails.model');
  ...
  exports.insert = (req, res) => {
        ...
        req.body.id=req.jwt.id
            ...
            UserDetailsModel.createUserDetails(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      UserDetailsModel.list(limit, page,req.query)
          .then((result) => {
  ...
  exports.listAll = (req, res ) => {
     ...
     */
    UserDetailsModel.listAll(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    UserDetailsModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    UserDetailsModel.findById(req.params.userdetailsId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      UserDetailsModel.patchUserDetails(req.params.userdetailsId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    UserDetailsModel.removeById(req.params.userdetailsId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
...
exports.uploadfile=(req,res)=>{
    UserDetailsModel.uploadFile(req)
        .then((result) => {
