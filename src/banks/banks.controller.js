const BanksModel = require('./banks.model');
  ...
  exports.insert = (req, res) => {
        ...
        req.body.id=req.jwt.id
            ...
            BanksModel.createBanks(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      BanksModel.list(limit, page,req.query)
          .then((result) => {
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    BanksModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    BanksModel.findById(req.params.banksId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      BanksModel.patchBanks(req.params.banksId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    BanksModel.removeById(req.params.banksId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
