const KycModel = require('./kyc.model');
  ...
  exports.list = (req, res ) => {
      ...
      KycModel.list(limit, page,req.query)
          .then((result) => {
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    KycModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    KycModel.findById(req.params.kycId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      KycModel.patchKyc(req.params.kycId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    KycModel.removeById(req.params.kycId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
